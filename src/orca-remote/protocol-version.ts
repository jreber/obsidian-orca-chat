...
// Vendoring note: the original file also imports/re-exports
// REMOTE_SERVER_UPDATE_CAPABILITY from './remote-server-update', which pulls
// in Orca's auto-updater type chain (update-status-types -> release-channel).
// That's unrelated to the E2EE handshake and session/chat RPC protocol this
// plugin vendors, so it has been dropped here (and below in
// RUNTIME_CAPABILITIES) to avoid vendoring the updater surface.
