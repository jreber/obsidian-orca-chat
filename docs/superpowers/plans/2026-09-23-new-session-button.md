...
- [ ] **Step 2: Write `test-e2e/new-session.spec.ts`** with these tests (each uses `server`, `obsidian`, and the fixture's `vaultPath`):
  ...
  6. **create refused:** `server.setCreateRefusal({code:"agent_session_unsupported", message:"nope"})`; click; notice contains "nope"; status "⚠ Session not created"; nothing stored (reload → "No session").
